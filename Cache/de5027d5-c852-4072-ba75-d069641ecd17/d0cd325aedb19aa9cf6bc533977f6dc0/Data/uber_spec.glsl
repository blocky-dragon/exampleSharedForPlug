...
SPEC_CONST(bool) ENABLE_MATERIAL_PARAM_TEX = false;
...
bool N7_EnableMaterialParamsTexture_evaluate() { return bool( ENABLE_MATERIAL_PARAM_TEX ); }
...
#define N7_system_srgbToLinear( value ) ssSRGB_to_Linear( value )
...
void N7_main()
{
	...
	//We removed Projected UV from transform UVs, there"s no way to hide the matrix parameter UI.
	...
	if(N7_EnableSimpleReflection){
		...
		simplereflectionColor = N7_system_srgbToLinear(simplereflectionColor);
	...
	if(N7_EnableRimHighlight){
		...
		rimHighlight += ssPow(rimFactor, N7_RimExponent) * N7_system_srgbToLinear(rimCol);
	...
	vec3 AO = (N7_VertexColorSwitch == 3) ? N7_system_getSurfaceColor().rgb : vec3(1.0);
	
	if(N7_EnableMaterialParamsTexture){
		vec2 Material_Params_UV = N7_getUV(N7_MaterialParamsUVSwitch);
		vec3 MaterialParams = N7_MaterialParamsTexture_sample(Material_Params_UV).rgb;
		N7_Metallic *= MaterialParams.r;
		N7_Roughness *= MaterialParams.g;
		AO *= MaterialParams.b;
	}
	
	if(N7_EnableSpecularAO){
		N7_SpecularAO = mix(vec3(0.04), N7_Albedo.rgb * N7_Metallic, N7_Metallic);
		N7_SpecularAO = mix((1.0 - N7_Darkening) * N7_SpecularAO * N7_SpecularAO , vec3(1.0), AO.r);
...
void Node7_Code_Node_Uber_Specular( in float VertexColorSwitch, in float4 BaseColorValue, in float EnableBaseTexture, in float BaseTextureUVSwitch, in float EnableRecolor, in float3 RecolorR, in float3 RecolorG, in float3 RecolorB, in float EnableOpacityTexture, in float OpacityUVSwitch, in float EnableNormalTexture, in float NormalUVSwitch, in float EnableDetailNormal, in float Detail_NormalUVSwitch, in float EnableEmissiveTexture, in float EmissiveUVSwitch, in float3 EmissiveColor, in float EmissiveIntensity, in float EnableSimpleReflection, in float ReflectionIntensity, in float EnableModulationTexture, in float ModuationUVSwitch, in float EnableRimHighlight, in float3 RimColor, in float RimIntensity, in float RimExponent, in float EnableRimColorTexture, in float EnableRimInvert, in float RimUVSwitch, in float EnableMaterialParamsTexture, in float MetallicValue, in float RoughnessValue, in float MaterialParamsUVSwitch, in float EnableSpecularAO, in float Intensity, in float Darkening, in float EnableUV2, in float UV2Switch, in float2 UV2Scale, in float2 UV2Offset, in float UV2Animation, in float EnableUV3, in float UV3Switch, in float2 UV3Scale, in float2 UV3Offset, in float UV3Animation, out float4 Albedo, out float Opacity, out float3 Normal, out float3 Emissive, out float Metallic, out float Roughness, out float3 SpecularAO, ssGlobals Globals )
{ 
	tempGlobals = Globals;
	...
	N7_EnableMaterialParamsTexture = bool( ENABLE_MATERIAL_PARAM_TEX );
...
void Node36_PBR_Lighting( in float3 Albedo, in float Opacity, in float3 Normal, in float3 Emissive, in float Metallic, in float Roughness, in float3 SpecularAO, out float4 Output, ssGlobals Globals )
{ 
	if ( !sc_ProjectiveShadowsCaster )
	{
...
void main() 
{
	...
	
	{
		...
		float Output_N8 = 0.0; Node8_Bool_Parameter( Output_N8, Globals );
		...
		float4 Albedo_N7 = float4(0.0); float Opacity_N7 = 0.0; float3 Normal_N7 = float3(0.0); float3 Emissive_N7 = float3(0.0); float Metallic_N7 = 0.0; float Roughness_N7 = 0.0; float3 SpecularAO_N7 = float3(0.0); Node7_Code_Node_Uber_Specular( Output_N38, Output_N5, Output_N121, Output_N27, Output_N37, Output_N85, Output_N86, Output_N87, Output_N308, Output_N69, Output_N354, Output_N181, Output_N218, Output_N184, Output_N223, Output_N76, Output_N236, Output_N233, Output_N179, Output_N225, Output_N177, Output_N228, Output_N74, Output_N309, Output_N310, Output_N311, Output_N216, Output_N312, Output_N315, Output_N8, Output_N242, Output_N243, Output_N221, Output_N219, Output_N244, Output_N245, Output_N67, Output_N13, Output_N14, Output_N15, Output_N16, Output_N11, Output_N49, Output_N50, Output_N51, Output_N52, Albedo_N7, Opacity_N7, Normal_N7, Emissive_N7, Metallic_N7, Roughness_N7, SpecularAO_N7, Globals );
		float4 Output_N36 = float4(0.0); Node36_PBR_Lighting( Albedo_N7.xyz, Opacity_N7, Normal_N7, Emissive_N7, Metallic_N7, Roughness_N7, SpecularAO_N7, Output_N36, Globals );
